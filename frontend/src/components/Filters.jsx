...
function Filters() {
  const { priority, setPriority } = useTaskStore();

  const [activeIndex, setActiveIndex] = useState(0); //for active tab
  ...
  return (
    <div className="relative py-2 px-2 grid grid-cols-4 items-center gap-3 bg-[#F9F9F9] border-2 border-white rounded-md">
      ...
      ></span>
      {priorities.map((priorityLabel, index) => (
        <button
          key={index}
          ...
          onClick={() => {
            ...
            setPriority(priorityLabel.toLowerCase());
          }}
        >
          {priorityLabel}
        </button>
